...
/**
 * Threads — animated WebGL "threads" background.
 * Vendored from react-bits (github.com/DavidHDev/react-bits, ts-tailwind variant),
 * MIT. Uses `ogl` directly (no three/r3f). Self-contained: visibility-gated,
 * render-resolution capped, cleans up its GL context on unmount.
 */
...
interface ThreadsProps {
  ...
  className?: string;
  /** When true, the render loop idles (no GPU pass). The canvas stays mounted,
   *  so there's no WebGL context loss / white flash from tearing it down. */
  paused?: boolean;
}
